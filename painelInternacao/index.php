...
<body>
	...
	<script>
		...
		function checkTime(i) {
			...
			//console.log(contador_new);

			if (nome == "" || contador_new > 3) {
				...
				if (nome == "") {
					...
					if (resultado == "") {
						$.ajax({
							...
							error: function(txt) {
								location.href = 'index.php?local=<?php echo $local; ?>';
							}
						});
					}
					if (resultado != "") {
			...
			} else if ((codlo == 5) || (codlo == 2) || (codlo == 7)) {
		...
		setInterval(buscaHistorico, 3000);
	</script>
</body>
